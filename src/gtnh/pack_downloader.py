...
log = logging.getLogger("pack_donwloader")
...
def download_mod(g: Github, o: Organization, mod: ModInfo) -> List[Path]:
    log.info("***********************************************************")
    log.info(f"Downloading {mod.name}:{mod.version}")
    paths = []
    repo = o.get_repo(mod.name)
    release: GitRelease = repo.get_release(mod.version)

    if not release:
        log.warning(f"*** No release found for {mod.name}:{mod.version}")
        return []
