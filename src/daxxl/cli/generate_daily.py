...
async def generate_daily(update_available: bool, new_id: int | None) -> None:
    async with httpx.AsyncClient(http2=True) as client:
        ...
        if new_id:
            ...
            m.counter.increment_daily_count()  # assets need to be uploaded even if the build crashes, it tracks the build id
        _, update_errors = await m.update_service.update_rolling_release(DevRelease.DAILY.value, update_available=update_available)
        if update_errors:
